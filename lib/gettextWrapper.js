    ...
    return {
        ...
        addTextDomain: function(domain, body, options) {
            ...
            for (var k in bareboneJson) {
                bareboneJson[k] = this.fixMetadata(bareboneJson[k], options);
            }
        ...
        fixMetadata: function(data, options) {
            ...
            for (var k in data) {
                ...
                if (k[0] === '\u0005') {
                    ...
                    var keys = k.slice(1).split(separator);
                    // update last key
                    var lastKey = keys.length - 1;
                    keys[lastKey] = metadataprefix + keys[lastKey];
                    var prefix = keys.join(separator);
