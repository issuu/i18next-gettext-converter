    ...
    return {
        ...
        i18nextToGettext: function(domain, source, target, options, callback) {
            ...
            self.readFile(source, options, function(err, body) {
                ...
                console.log('GIRAF', gt.domains[domain]);

                var data = '';
                switch (path.extname(target)) {
                    case '.po':
                        console.log('DEBUG', gt);
        ...
        parseJSON: function(domain, data, options) {
            ...
            for (var m in data) {
                ...
                for (var key in context) {
                    ...
                    if (values.comments) {
                        var _comments = {};
                        for (var _comments_key in values.comments) {
                            _comments[_comments_key] = toArrayIfNeeded(values.comments[_comments_key]);
                        }
                        appendTo[options.metadataprefix + targetKey] = _comments;
                    }
                }
